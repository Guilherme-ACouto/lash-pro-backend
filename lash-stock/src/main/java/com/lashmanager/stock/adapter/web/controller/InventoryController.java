...
public class InventoryController {
    ...
    public ResponseEntity<InventoryMovementResult> registerExit(
        ...
        var result = registerManualExitApplicationService.when(
                new RegisterManualExitCommand(id, req.quantity(), req.reason(), req.notes(), req.exitDate()));
        return ResponseEntity.status(201).body(result);
    }

    @GetMapping("/{id}/movements")
    public ResponseEntity<Page<InventoryMovementResult>> listMovements(@PathVariable UUID id, Pageable pageable) {
