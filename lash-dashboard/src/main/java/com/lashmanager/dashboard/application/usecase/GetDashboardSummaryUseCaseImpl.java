...
public class GetDashboardSummaryUseCaseImpl implements GetDashboardSummaryUseCase {
    ...
    public DashboardSummary execute() {
        ...
        long newClientsThisMonth = clientJpaRepository.findAllFiltered(null, true, Pageable.unpaged()).stream()
                .filter(c -> c.getCreatedAt() != null
                        && !c.getCreatedAt().toLocalDate().isBefore(startOfMonth))
                .count();

        long appointmentsToday =
                appointmentJpaRepository.findActiveByDate(today).size();
        long appointmentsThisWeek = appointmentJpaRepository.findByDateRange(startOfWeek, endOfWeek).stream()
                .filter(a -> !"CANCELLED".equals(a.getStatus()))
                .count();
        long appointmentsThisMonth = appointmentJpaRepository.findByDateRange(startOfMonth, endOfMonth).stream()
                .filter(a -> !"CANCELLED".equals(a.getStatus()))
                .count();

        var monthEntries = financialEntryJpaRepository.findWithFilters(
    ...
    private List<MonthlyRevenueStat> buildLast6Months(LocalDate today) {
        ...
        for (int i = 5; i >= 0; i--) {
            ...
            var entries = financialEntryJpaRepository.findWithFilters(start, end, null, null, null, Pageable.unpaged());
