...
public class GetDashboardSummaryUseCaseImpl implements GetDashboardSummaryUseCase {
    ...
    public DashboardSummary execute() {
        ...
        long newClientsThisMonth = clientJpaRepository
                .findAllFiltered(null, true, Pageable.unpaged()).stream()
        ...
        long appointmentsThisWeek = appointmentJpaRepository
                .findByDateRange(startOfWeek, endOfWeek).stream()
                .filter(a -> !"CANCELLED".equals(a.getStatus()))
                .count();
        long appointmentsThisMonth = appointmentJpaRepository
                .findByDateRange(startOfMonth, endOfMonth).stream()
                .filter(a -> !"CANCELLED".equals(a.getStatus()))
                .count();

        var monthEntries = financialEntryJpaRepository
                .findWithFilters(startOfMonth, endOfMonth, null, null, null, Pageable.unpaged());
        BigDecimal revenueThisMonth = monthEntries.stream()
                .filter(e -> "INCOME".equals(e.getType()))
                .map(e -> e.getAmount())
                .reduce(BigDecimal.ZERO, BigDecimal::add);
        BigDecimal expenseThisMonth = monthEntries.stream()
                .filter(e -> "EXPENSE".equals(e.getType()))
                .map(e -> e.getAmount())
                .reduce(BigDecimal.ZERO, BigDecimal::add);

        long lowStockItems = inventoryItemJpaRepository.findAll().stream()
                .filter(i -> i.isActive()
                        && i.getCurrentQuantity().compareTo(BigDecimal.ZERO) > 0
                        && i.getCurrentQuantity().compareTo(i.getMinimumQuantity()) < 0)
                .count();
        long outOfStockItems = inventoryItemJpaRepository.findAll().stream()
                .filter(i -> i.isActive()
                        && i.getCurrentQuantity().compareTo(BigDecimal.ZERO) == 0)
                .count();
        ...
        return new DashboardSummary(
                ...
                revenueThisMonth, expenseThisMonth,
    ...
    private List<MonthlyRevenueStat> buildLast6Months(LocalDate today) {
        ...
        for (int i = 5; i >= 0; i--) {
            ...
            var entries = financialEntryJpaRepository
                    .findWithFilters(start, end, null, null, null, Pageable.unpaged());
