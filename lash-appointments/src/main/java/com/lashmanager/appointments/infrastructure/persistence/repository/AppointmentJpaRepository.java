...
public interface AppointmentJpaRepository extends JpaRepository<AppointmentEntity, UUID> {
    ...
    @Query("SELECT a FROM AppointmentEntity a WHERE a.scheduledDate = :date AND a.status != 'CANCELLED' ORDER BY a.scheduledTime")
    ...
    @Query("SELECT a FROM AppointmentEntity a WHERE a.scheduledDate >= :startDate AND a.scheduledDate <= :endDate ORDER BY a.scheduledDate, a.scheduledTime")
    List<AppointmentEntity> findByDateRange(@Param("startDate") LocalDate startDate, @Param("endDate") LocalDate endDate);

    @Query("SELECT a FROM AppointmentEntity a WHERE a.clientId = :clientId AND a.scheduledDate > :from AND a.status IN ('SCHEDULED','CONFIRMED') ORDER BY a.scheduledDate, a.scheduledTime")
    List<AppointmentEntity> findFutureActiveByClientId(@Param("clientId") UUID clientId, @Param("from") LocalDate from);

    @Query("SELECT COUNT(a) FROM AppointmentEntity a WHERE a.serviceId = :serviceId AND a.scheduledDate >= :from AND a.status IN ('SCHEDULED','CONFIRMED')")
