...
public interface AppointmentJpaRepository extends JpaRepository<AppointmentEntity, UUID> {
  ...
  List<AppointmentEntity> findFutureActiveByClientId(
      @Param("clientId") UUID clientId, @Param("from") LocalDate from);
  ...
  @Transactional
  @Query(
      "DELETE FROM AppointmentEntity a WHERE a.clientId = :clientId AND a.scheduledDate >= :from")
  void deleteFutureByClientId(@Param("clientId") UUID clientId, @Param("from") LocalDate from);

  @Modifying
  @Transactional
  @Query(
      "UPDATE AppointmentEntity a SET a.clientId = null WHERE a.clientId = :clientId AND a.scheduledDate < :from")
  void unlinkClientFromPastAppointments(
      @Param("clientId") UUID clientId, @Param("from") LocalDate from);
}
