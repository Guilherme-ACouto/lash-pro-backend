...
public class DeactivateServiceUseCaseImpl implements DeactivateServiceUseCase {
    ...
    public void deactivate(UUID id, boolean force) {
        ServiceOffering service = serviceRepository.findById(id).orElseThrow(() -> new ServiceNotFoundException(id));
    ...
    public void reactivate(UUID id) {
        ...
        serviceRepository.save(
                service.toBuilder().active(true).updatedAt(LocalDateTime.now()).build());
    }
}
